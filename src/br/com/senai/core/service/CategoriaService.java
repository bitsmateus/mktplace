...
public class CategoriaService {
	...
	public void removerPor(int id) {
		if(id > 0) {
			int qtdeDeRestaurante = daoRestaurante.contarPor(id);
			if(qtdeDeRestaurante > 0) {
				throw new IllegalArgumentException("Não foi possivel exlcluir a catergoria"
						+ "Motivo: Existe(m) " + qtdeDeRestaurante + "vinculados a categoria");
			}
